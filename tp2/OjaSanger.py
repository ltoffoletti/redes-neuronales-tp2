...
class redOjaSanger(object):
        ...
        def OjaSanger(self,n,L_in,n_train=0.05,w_tolerance=0.001,w_init=50,OjaSanger=0):
                ...
                L_in_interna = list(np.copy(L_in));

                #Primero inicializo los pesos de los componentes principales
                self.w_OjaSanger = np.array([[random.randint(-w_init,w_init)/100 for x in range(len(L_in_interna[0]))] for y in range(n)]);
                ...
                while b:
                        ...
                        if i%len(L_in_interna) == 0:
                                random.shuffle(L_in_interna);
                                if self.verbose >= 7 and i > 0:
                                        print('Iteracion numero ' + str(i));

                        #current representa el input actual
                        current = L_in_interna[i%len(L_in_interna)];
                        ...
                        b = i < int(math.sqrt(len(L_in_interna))) or self._w_OjaSanger_check(w_tolerance);
